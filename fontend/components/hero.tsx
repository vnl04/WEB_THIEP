...
const showcaseCards = [
  { bg: 'from-amber-100 to-orange-50', name: 'Ngọc Lan & Minh Khang', date: '15.06.2025', templateId: 'template-1' },
  { bg: 'from-rose/20 to-pink/10', name: 'Anh Tuấn & Linh Chi', date: '22.07.2025', templateId: 'template-2' },
  { bg: 'from-slate-100 to-gray-50', name: 'Đức Anh & Thảo Nhi', date: '08.08.2025', templateId: 'template-3' },
  { bg: 'from-yellow-100 to-amber-50', name: 'Văn Nam & Hương Giang', date: '14.09.2025', templateId: 'template-4' },
]
    ...
    <section className="relative w-full min-h-fit pt-20 pb-8 px-4 overflow-hidden bg-gradient-to-b from-rose/10 via-pink/5 to-white">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-7 fade-in" style={{ animation: 'fadeInUp 0.6s ease-out 0.2s forwards', opacity: 0 }}>
          <Link href={ROUTES.register} className="btn-primary text-lg hover:shadow-xl transform hover:-translate-y-1">
            Tạo thiệp miễn phí →
          </Link>
          <Link href={SECTIONS.templates} className="btn-secondary hover:shadow-lg">
            🎨 Xem mẫu thiệp
          </Link>
        </div>
            ...
            >
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                  <Link
                    href={`${ROUTES.register}?template=${card.templateId}`}
